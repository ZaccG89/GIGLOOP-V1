...
export default function Profile() {
  ...
  const { data: savedEvents = [], isLoading: savedLoading } = useQuery<Event[]>({
    ...
    queryFn: async () => {
      const r = await fetch("/api/saves/events", { credentials: "include" });
      if (!r.ok) return [];
      return r.json();
    },
    enabled: !!user,
  ...
  const { data: likedEvents = [], isLoading: likedLoading } = useQuery<Event[]>({
    ...
    queryFn: async () => {
      const r = await fetch("/api/likes/events", { credentials: "include" });
      if (!r.ok) return [];
      return r.json();
    },
    enabled: !!user,
  ...
  const { data: artists = [], isLoading: artistsLoading } = useQuery<UserArtist[]>({
    ...
    queryFn: async () => {
      const r = await fetch("/api/user/artists", { credentials: "include" });
      if (!r.ok) return [];
      return r.json();
    },
    enabled: !!user,
  ...
  const { data: followRequests = [] } = useQuery<(Follow & { follower: User })[]>({
    ...
    queryFn: async () => {
      const r = await fetch("/api/follows/requests", { credentials: "include" });
      if (!r.ok) return [];
      return r.json();
    },
    enabled: !!user,
  ...
  const { data: following = [] } = useQuery<(Follow & { following: User })[]>({
    ...
    queryFn: async () => {
      const r = await fetch("/api/follows/following", { credentials: "include" });
      if (!r.ok) return [];
      return r.json();
    },
    enabled: !!user,
        ...
        <div className="max-w-xl mx-auto py-16 px-6">
          ...
          >
            <div
              ...
              style={{ background: "rgba(155,92,255,0.12)" }}
            >
              <Lock className="w-8 h-8" style={{ color: "var(--purple)" }} />
            </div>

            <h1 className="text-3xl font-extrabold text-white mb-2">Create your profile</h1>
            <p className="text-sm mb-6" style={{ color: "var(--muted-color)" }}>
              Save gigs, like events, build your artist profile, and connect with other music lovers.
            </p>
    ...
    <Layout>
      <div className="mb-8">
        <div className="flex items-start gap-5 mb-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            {u.username && (
              <p className="text-sm mt-0.5" style={{ color: "var(--purple)" }}>
                @{u.username}
              </p>
            )}

            {u.bio && (
              <p className="text-sm mt-1 line-clamp-2" style={{ color: "var(--muted-color)" }}>
                {u.bio}
              </p>
            )}
          ...
          <button
            ...
            style={{
              ...
              color: editing ? "#05060A" : "var(--silver)",
              border: "1px solid var(--border-raw)",
            }}
          >
        ...
        {editing && (
          ...
          >
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <Field label="Username">
                <div className="flex items-center">
                  <span
                    className="px-3 py-2.5 rounded-l-xl text-sm border border-r-0 border-white/10 bg-white/5"
                    style={{ color: "var(--muted-color)" }}
                  >
                    @
                  </span>
                  <input
                    className="g-input rounded-l-none flex-1"
                    value={form.username}
                    onChange={(e) =>
            ...
            <Field label="Bio">
              <textarea
                className="g-input min-h-[80px] resize-none"
                value={form.bio}
              ...
              <div className="flex items-center gap-4">
                <div className="relative flex-shrink-0 w-16 h-16">
                  {form.avatarUrl ? (
                    ...
                    <div
                      className="w-16 h-16 rounded-xl flex items-center justify-center text-2xl font-extrabold text-white"
                      style={{ background: "linear-gradient(135deg, var(--purple), #7B3FD8)" }}
                    >
                      {initial}
                    </div>
            ...
            >
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: "var(--muted-color)" }}>
                  Others must request to follow you before seeing your liked events
                </p>
              </div>
            ...
            >
              ...
              {t.count !== undefined && t.count > 0 && (
                <span
                  ...
                  style={{
                    background:
                      tab === t.id ? "rgba(0,0,0,0.2)" : "rgba(155,92,255,0.2)",
      ...
      {tab === "saved" && (
        <EventList
          ...
          emptyIcon={Bookmark}
          emptyText="No saved gigs yet"
          emptyHint="Hit the bookmark icon on any gig card to save it here."
        />
      )}
      ...
      {tab === "liked" && (
        <EventList
          ...
          emptyIcon={Heart}
          emptyText="No liked gigs yet"
          emptyHint="Hit the heart icon on any gig card."
        />
      )}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold truncate" style={{ color: "var(--silver)" }}>
                    {req.follower.displayName || "Unknown"}
                  </p>
                  {(req.follower as any).username && (
                ...
                <div className="flex gap-2 flex-shrink-0">
                  <button
                    onClick={() =>
                      respondToFollow.mutate({ followId: req.id, status: "accepted" })
                    }
                  ...
                  <button
                    onClick={() =>
                      respondToFollow.mutate({ followId: req.id, status: "rejected" })
                    }
      ...
      >
        {event.imageUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <Music className="w-6 h-6" style={{ color: "var(--border-raw)" }} />
          </div>
        )}
      </div>
