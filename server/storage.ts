import { db } from "./db";
import {
  users, spotifyAccounts, appleAccounts, userArtists, venues, events, eventArtists, gigSubmissions,
  userSaves, userLikes, follows, userShares,
  type User, type InsertUser,
  type SpotifyAccount, type InsertSpotifyAccount,
  type AppleAccount, type InsertAppleAccount,
  type UserArtist, type InsertUserArtist,
  type Venue, type InsertVenue,
  type Event, type InsertEvent,
  type EventArtist, type InsertEventArtist,
  type GigSubmission, type InsertGigSubmission,
  type UserSave, type UserLike,
  type Follow,
} from "@shared/schema";
import { eq, and, or, ilike, gte, lte, sql, inArray, count } from "drizzle-orm";
...
export interface IStorage {
  ...
  registerVenueAccount(userId: string, venueData: Partial<InsertVenue>): Promise<Venue>;
  getVenueByOwnerId(userId: string): Promise<Venue | undefined>;
  getPendingVenues(): Promise<Venue[]>;
  updateVenueVerificationStatus(venueId: string, status: string): Promise<Venue>;
  ...
  // Profile
  updateUserProfile(userId: string, fields: { displayName?: string; username?: string; bio?: string; avatarUrl?: string; isPrivate?: boolean }): Promise<User>;
  ...
  respondToFollow(followId: string, status: "accepted" | "rejected"): Promise<Follow>;
  ...
  getEventCounts(eventId: string): Promise<{ likes: number; shares: number }>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getUserBySpotifyId(spotifyId: string): Promise<User | undefined> {
    const [account] = await db.select().from(spotifyAccounts).where(eq(spotifyAccounts.spotifyUserId, spotifyId));
  ...
  async getSpotifyAccount(userId: string): Promise<SpotifyAccount | undefined> {
    const [account] = await db.select().from(spotifyAccounts).where(eq(spotifyAccounts.userId, userId));
    return account;
  }

  async upsertSpotifyAccount(account: InsertSpotifyAccount): Promise<SpotifyAccount> {
    const [upserted] = await db.insert(spotifyAccounts).values(account)
      .onConflictDoUpdate({
  ...
  async getAppleAccount(userId: string): Promise<AppleAccount | undefined> {
    const [account] = await db.select().from(appleAccounts).where(eq(appleAccounts.userId, userId));
    return account;
  }

  async getUserByAppleToken(musicUserToken: string): Promise<User | undefined> {
    const [account] = await db.select().from(appleAccounts).where(eq(appleAccounts.musicUserToken, musicUserToken));
    if (!account) return undefined;
    return this.getUser(account.userId);
  }

  async upsertAppleAccount(account: InsertAppleAccount): Promise<AppleAccount> {
    const [upserted] = await db.insert(appleAccounts).values(account)
      .onConflictDoUpdate({
  ...
  async getUserArtists(userId: string): Promise<UserArtist[]> {
    return await db.select().from(userArtists).where(eq(userArtists.userId, userId));
  }

  async upsertUserArtists(artists: InsertUserArtist[]): Promise<void> {
    if (artists.length === 0) return;
    await db.insert(userArtists).values(artists)
      .onConflictDoUpdate({
  ...
  async removeUserArtist(userId: string, spotifyArtistId: string): Promise<void> {
    await db.delete(userArtists).where(
      and(eq(userArtists.userId, userId), eq(userArtists.spotifyArtistId, spotifyArtistId))
    );
  }
  ...
  async upsertVenue(venue: InsertVenue): Promise<Venue> {
    const [upserted] = await db.insert(venues).values(venue)
      .onConflictDoUpdate({
        target: venues.id,
        set: venue
      }).returning();
  ...
  async registerVenueAccount(userId: string, venueData: Partial<InsertVenue>): Promise<Venue> {
    // Check if user already owns a venue
    const existing = await this.getVenueByOwnerId(userId);
    if (existing) {
      // Update their venue profile and resubmit for verification
      const [updated] = await db.update(venues).set({
        ...venueData,
        ownerId: userId,
        verificationStatus: "pending",
      }).where(eq(venues.id, existing.id)).returning();
      return updated;
    }
    // Create a new venue linked to this user
  ...
  async getVenueByOwnerId(userId: string): Promise<Venue | undefined> {
    const [venue] = await db.select().from(venues).where(eq(venues.ownerId, userId));
    return venue;
  }
  ...
  async updateVenueVerificationStatus(venueId: string, status: string): Promise<Venue> {
    const set: Partial<typeof venues.$inferInsert> = { verificationStatus: status };
    if (status === "approved") {
      set.isActive = true;
      set.verifiedAt = new Date();
    }
    const [updated] = await db.update(venues).set(set).where(eq(venues.id, venueId)).returning();
    return updated;
  }

  async getEventsByVenueName(venueName: string, days = 42): Promise<Event[]> {
  ...
  async upsertEvent(event: InsertEvent): Promise<Event> {
    const [upserted] = await db.insert(events).values(event)
      .onConflictDoUpdate({
  ...
  async getEventArtists(providerEventId: string): Promise<EventArtist[]> {
    return await db.select().from(eventArtists).where(eq(eventArtists.providerEventId, providerEventId));
  }

  async upsertEventArtists(artists: InsertEventArtist[]): Promise<void> {
    if (artists.length === 0) return;
    await db.insert(eventArtists).values(artists)
      .onConflictDoNothing();
  }

  async createGigSubmission(submission: InsertGigSubmission): Promise<GigSubmission> {
    const [created] = await db.insert(gigSubmissions).values(submission).returning();
    return created;
  }

  async getPendingGigSubmissions(): Promise<GigSubmission[]> {
    return await db.select().from(gigSubmissions).where(eq(gigSubmissions.status, 'pending'));
  }

  async getGigSubmission(id: string): Promise<GigSubmission | undefined> {
    const [submission] = await db.select().from(gigSubmissions).where(eq(gigSubmissions.id, id));
    return submission;
  }

  async updateGigSubmissionStatus(id: string, status: string): Promise<GigSubmission> {
    const [updated] = await db.update(gigSubmissions).set({ status }).where(eq(gigSubmissions.id, id)).returning();
    return updated;
  }

  async toggleSave(userId: string, eventId: string): Promise<{ saved: boolean }> {
    const [existing] = await db.select().from(userSaves)
      .where(and(eq(userSaves.userId, userId), eq(userSaves.eventId, eventId)));
    if (existing) {
      await db.delete(userSaves).where(and(eq(userSaves.userId, userId), eq(userSaves.eventId, eventId)));
      return { saved: false };
    }
  ...
  async toggleLike(userId: string, eventId: string): Promise<{ liked: boolean }> {
    const [existing] = await db.select().from(userLikes)
      .where(and(eq(userLikes.userId, userId), eq(userLikes.eventId, eventId)));
    if (existing) {
      await db.delete(userLikes).where(and(eq(userLikes.userId, userId), eq(userLikes.eventId, eventId)));
      return { liked: false };
    }
  ...
  async getUserSaveIds(userId: string): Promise<string[]> {
    const rows = await db.select({ eventId: userSaves.eventId }).from(userSaves)
      .where(eq(userSaves.userId, userId));
    return rows.map(r => r.eventId);
  }

  async getUserLikeIds(userId: string): Promise<string[]> {
    const rows = await db.select({ eventId: userLikes.eventId }).from(userLikes)
      .where(eq(userLikes.userId, userId));
    return rows.map(r => r.eventId);
  }

  async getUserSavedEvents(userId: string): Promise<Event[]> {
    const saves = await db.select({ eventId: userSaves.eventId })
      .from(userSaves).where(eq(userSaves.userId, userId));
    if (saves.length === 0) return [];
    return await db.select().from(events).where(inArray(events.id, saves.map(s => s.eventId)));
  }

  async getUserLikedEvents(userId: string): Promise<Event[]> {
    const likes = await db.select({ eventId: userLikes.eventId })
      .from(userLikes).where(eq(userLikes.userId, userId));
    if (likes.length === 0) return [];
    return await db.select().from(events).where(inArray(events.id, likes.map(l => l.eventId)));
  }

  // ── Profile ───────────────────────────────────────────────────────────────
  async updateUserProfile(userId: string, fields: { displayName?: string; username?: string; bio?: string; avatarUrl?: string; isPrivate?: boolean }): Promise<User> {
  ...
  // ── Follows ───────────────────────────────────────────────────────────────
  async sendFollowRequest(followerId: string, followingId: string): Promise<Follow> {
    ...
    if (existing) {
      if (existing.status === "rejected") {
        const [updated] = await db.update(follows).set({ status }).where(eq(follows.id, existing.id)).returning();
        return updated;
      }
      return existing;
    }
    const [created] = await db.insert(follows).values({ followerId, followingId, status }).returning();
    return created;
  }

  async respondToFollow(followId: string, status: "accepted" | "rejected"): Promise<Follow> {
    const [updated] = await db.update(follows).set({ status }).where(eq(follows.id, followId)).returning();
    return updated;
  }
  ...
  // ── Shares ────────────────────────────────────────────────────────────────
  async toggleShare(userId: string, eventId: string): Promise<{ shared: boolean }> {
    const [existing] = await db.select().from(userShares)
      .where(and(eq(userShares.userId, userId), eq(userShares.eventId, eventId)));
    if (existing) {
      await db.delete(userShares).where(and(eq(userShares.userId, userId), eq(userShares.eventId, eventId)));
      return { shared: false };
    }
  ...
  async getUserShareIds(userId: string): Promise<string[]> {
    const rows = await db.select({ eventId: userShares.eventId }).from(userShares)
      .where(eq(userShares.userId, userId));
    return rows.map(r => r.eventId);
  }

  async getEventCounts(eventId: string): Promise<{ likes: number; shares: number }> {
    const [likesRow] = await db.select({ n: count() }).from(userLikes).where(eq(userLikes.eventId, eventId));
    const [sharesRow] = await db.select({ n: count() }).from(userShares).where(eq(userShares.eventId, eventId));
    return { likes: Number(likesRow?.n ?? 0), shares: Number(sharesRow?.n ?? 0) };
  }
}
